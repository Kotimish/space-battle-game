...
Диаграмма иллюстрирует ключевые этапы взаимодействия между Агентом Игроком, сервисом Auth и сервисом Game Server: 
...
Для запуска тестов с измерением покрытия используйте соответствующие команды в директории каждого микросервиса.

```bash
pytest --cov=src --cov-report=markdown
```

Покрытие тестов для каждого микросервиса оценивается отдельно.

Отчёты о покрытии генерируются в формате Markdown и находятся в корневых папках соответствующих микросервисов:

*   **Auth Service:** [`auth_service/coverage.md`](./apps/auth/coverage.md)
*   **Game Server:** [`game_server/coverage.md`](./apps/game-server/coverage.md)