...
def test_create_duplicate_game_id():
    ...
    session1 = AuthGameSession("game_1", ["any_data_1"])
    repository.create(session1)
    session2 = AuthGameSession("game_1", ["any_data_2"])
...
def test_get_all_game_ids():
    ...
    repository = InMemoryAuthGameSessionRepository()
    repository.create(AuthGameSession("game_1", ["any_data_1"]))
    repository.create(AuthGameSession("game_2", ["any_data_2"]))
